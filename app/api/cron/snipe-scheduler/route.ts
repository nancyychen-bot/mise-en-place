...
export async function GET() {
  const { data: restaurants, error } = await db
    .from('restaurants')
    .select('id, user_id, name, venue_id, platform, party_size, party_sizes, release_days_ahead, release_time, preferred_time, date_start, date_end, earliest_time, day_range')
    .eq('active', true)
    .eq('auto_book', true)
  ...
  for (const r of restaurants) {
    ...
    const dispatchRes = await fetch(
      'https://api.github.com/repos/nancyychen-bot/mise-en-place/actions/workflows/opentable-snipe.yml/dispatches',
      {
        method: 'POST',
        headers: {
